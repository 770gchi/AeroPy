...
class Invincibility:
    ...
    def invincible_mode_event(self, event):
        _, _, m_right = pygame.mouse.get_pressed()
        return m_right

    def mode_switch(self):
        if self.invincible_mode_event(event):
            self.flag = not self.flag  # 切换无敌模式开关
            print(f"无敌模式: {'开启' if self.invincible else '关闭'}")  # 打印无敌状态
...
class Player(Entity):
    def __init__(self, config: GameConfig) -> None:
        image = config.images.player
        image_invincible = config.images.invincible
    ...
    def tick_crash(self) -> None:
        if self.min_y <= self.y <= self.max_y:
            self.y = clamp(self.y + self.vel_y, self.min_y, self.max_y)
            # rotate only when it's a biru crash and bird is still falling
    ...
    def crossed(self, biru: Biru) -> bool:
        return biru.cx <= self.cx < biru.cx - biru.vel_x

    def collided(self, birus: Birus, floor: Floor) -> bool:
        """returns True if player collides with floor or birus."""
        ...
        for biru in birus.upper:
            if self.collide(biru):
                self.crashed = True
                self.crash_entity = "biru"
                return True
        for biru in birus.lower:
            if self.collide(biru):
                self.crashed = True
                self.crash_entity = "biru"
                return True
    ...
    def img_switch(self):
        if Invincibility.flag:
            self.image = self.config.images.invincible
        else:
            self.image = self.config.images.player
